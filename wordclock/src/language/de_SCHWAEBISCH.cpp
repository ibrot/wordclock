...
void Grid_de_SCHWAEBISCH::setTime(int hour, int minute) {
  ...
  for(int i = 0; i < sizeof(Grid_de_SCHWAEBISCH::time_it_is) / sizeof(Grid_de_SCHWAEBISCH::time_it_is[0]); i++) {
    Led::ids[Led::getLedId(Grid_de_SCHWAEBISCH::time_it_is[i])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
  }

  for(int m = 0; m < sizeof(Grid_de_SCHWAEBISCH::time_minutes[minute]) / sizeof(Grid_de_SCHWAEBISCH::time_minutes[minute][0]); m++) {
